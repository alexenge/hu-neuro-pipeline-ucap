...
```{r, fig3, fig.width=7.5, fig.height=2.5}
...
# Create topoplot for a single time window (N2 component) 
evokeds_diff %>%
  # Select time window
  filter(time >= 0.25 & time < 0.35) %>%
...
# Repeat for multiple time windows
# Here: 0 ms to 800 ms in windows of 200 ms
tmins <- seq(0.0, 0.6, by = 0.1)
...
cowplot::plot_grid(plotlist = topoplots, nrow = 1)
```
